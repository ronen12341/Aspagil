...
function buildEmailHtml(orderId, body) {
  ...
  const itemRows = (items || []).map(i => {
    ...
    return `<tr style="border-bottom:1px solid #eee;">
      <td style="padding:10px 8px;color:#3B1F0A;"><strong>${escapeHtml(i.name)}</strong>${i.note ? `<div style="font-size:11px;color:#888">${escapeHtml(i.note)}</div>` : ""}</td>
      <td style="padding:10px 8px;color:#3B1F0A;text-align:center;">${i.qty}</td>
      <td style="padding:10px 8px;color:#3B1F0A;text-align:end;">${escapeHtml(i.price || "—")}</td>
...
<div dir="rtl" style="font-family:Arial,sans-serif;max-width:700px;margin:0 auto;background:#F4F4F4;padding:24px;border-radius:8px;">
  <h2 style="color:#3B1F0A;border-bottom:2px solid #C8922A;padding-bottom:8px;margin-top:0;">🛒 הזמנה חדשה מאתר אספגיל</h2>
  ...
  <h3 style="color:#A87520;margin-top:24px;">פרטי לקוח</h3>
  <table style="width:100%;border-collapse:collapse;background:#fff;border-radius:8px;overflow:hidden;">
    <tr><td style="padding:10px;color:#A87520;font-weight:bold;width:140px;">שם</td><td style="padding:10px;">${escapeHtml(customer.name)}</td></tr>
    ${customer.business ? `<tr style="background:#FAF7F2;"><td style="padding:10px;color:#A87520;font-weight:bold;">עסק</td><td style="padding:10px;">${escapeHtml(customer.business)}</td></tr>` : ""}
    <tr style="background:#FAF7F2;"><td style="padding:10px;color:#A87520;font-weight:bold;">טלפון</td><td style="padding:10px;direction:ltr;"><a href="tel:${escapeHtml(customer.phone)}" style="color:#C8922A;">${escapeHtml(customer.phone)}</a> · <a href="${waLink}" style="color:#25D366;">וואטסאפ ↗</a></td></tr>
    ${customer.email ? `<tr><td style="padding:10px;color:#A87520;font-weight:bold;">אימייל</td><td style="padding:10px;direction:ltr;">${escapeHtml(customer.email)}</td></tr>` : ""}
    ${customer.taxId ? `<tr style="background:#FAF7F2;"><td style="padding:10px;color:#A87520;font-weight:bold;">ח.פ / ת.ז</td><td style="padding:10px;direction:ltr;">${escapeHtml(customer.taxId)}</td></tr>` : ""}
    ${customer.invoiceName ? `<tr><td style="padding:10px;color:#A87520;font-weight:bold;">שם לחשבונית</td><td style="padding:10px;">${escapeHtml(customer.invoiceName)}</td></tr>` : ""}
    ${customer.address || customer.city ? `<tr style="background:#FAF7F2;"><td style="padding:10px;color:#A87520;font-weight:bold;">כתובת</td><td style="padding:10px;">${escapeHtml([customer.address, customer.city].filter(Boolean).join(", "))}</td></tr>` : ""}
    ${customer.notes ? `<tr><td style="padding:10px;color:#A87520;font-weight:bold;vertical-align:top;">הערות</td><td style="padding:10px;white-space:pre-wrap;">${escapeHtml(customer.notes)}</td></tr>` : ""}
  </table>

  <h3 style="color:#A87520;margin-top:24px;">פריטי הזמנה</h3>
  <table style="width:100%;border-collapse:collapse;background:#fff;border-radius:8px;overflow:hidden;">
    <thead>
    ...
    <tbody>${itemRows}
      ${shipping && shipping.units > 0 ? `<tr style="border-bottom:1px solid #eee;">
        <td style="padding:10px 8px;color:#3B1F0A;"><strong>${isPickup ? "איסוף עצמי" : "משלוח"}</strong><div style="font-size:11px;color:#888">${shipping.units.toLocaleString("he-IL")} יח' בהזמנה</div></td>
        <td style="padding:10px 8px;color:#3B1F0A;text-align:center;">—</td>
        <td style="padding:10px 8px;color:#3B1F0A;text-align:end;">—</td>
        <td style="padding:10px 8px;color:#3B1F0A;text-align:end;font-weight:bold;">${isPickup ? "ללא עלות" : (shipping.needsArrangement ? "לפי תיאום" : Number(shipping.cost).toLocaleString("he-IL") + ' ש"ח')}</td>
      </tr>` : ""}
    </tbody>
    ${totalPrice > 0 ? `
    <tfoot>
  ...
  ${hasUnpricedItems ? `<p style="background:#FFF4D6;border-right:4px solid #C8922A;padding:12px;margin-top:16px;color:#7A4E00;">⚠️ יש פריטים ללא מחיר — צריך לקבוע מחיר בשיחה עם הלקוח.</p>` : ""}
  ${shipping && shipping.needsArrangement ? `<p style="background:#FFF4D6;border-right:4px solid #C8922A;padding:12px;margin-top:16px;color:#7A4E00;">🚚 הזמנה מעל 5,000 יח' — צריך לתאם עלות משלוח מול הלקוח בנפרד.</p>` : ""}

  <div style="margin-top:24px;padding:16px;background:#fff;border-radius:8px;text-align:center;">
    <p style="margin:0 0 8px;color:#A87520;font-weight:bold;">פעולות מהירות:</p>
    <a href="tel:${escapeHtml(customer.phone)}" style="display:inline-block;background:#A87520;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;margin:4px;">📞 התקשר ללקוח</a>
...
<h2 style="color:#C8922A;margin-top:0;">תודה על הזמנתכם — אספגיל</h2>
